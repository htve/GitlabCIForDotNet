...
{   
	...
	if([String]::IsNullOrEmpty($PublishProfile)){
      $result = ."$build" $projectPath $toolsVersion /p:RunCodeAnalysis=false /consoleloggerparameters:ErrorsOnly /p:Configuration=$Configuration /nologo /verbosity:quiet /maxcpucount 
	}
	else{
	  $result = ."$build" $projectPath $toolsVersion /p:RunCodeAnalysis=false /consoleloggerparameters:ErrorsOnly /p:Configuration=$Configuration /nologo /verbosity:quiet /maxcpucount /p:DeployOnBuild=true /p:publishUrl=$global:CiProjectPath/DeployArtifacts /p:PublishProfile=$PublishProfile
	}
